...
const ModalFinanceInformation: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <button
        onClick={() => {
          setIsShowFinanceInfo(!isShowFinanceInfo);
        }}
      ...
      <Typography className="font-bold text-lg">{financingType}</Typography>
      ...
      <div className="my-4 md:my-2 flex flex-col gap-2 font-poppins">
        <div
          dangerouslySetInnerHTML={{
            __html:
              t(
                `${pathTranslation}.${
                  financingType === 'Invoice Financing' ? 'text8' : 'text9'
                }`
              ) ?? ''
          }}
        />
        <a
          href={
            'https://danamart.id/blog/2024/06/po-financing-dan-invoice-financing-solusi-investasi-dan-tambah-modal-usaha/'
          }
