import { changeBankAccount, getBankList } from "@/repository/danamart/setting.repository";
...
const ChangeBankAccount: React.FC = () => {
  ...
  const [confirmNewBankAccountNameError, setConfirmNewBankAccountNameError] = useState<string | null>(null);
  const [confirmPasswordError, setConfirmPasswordError] = useState<string | null>(null);
  const [confirmOtpTypeError, setConfirmOtpTypeError] = useState<string | null>(null);
  const [confirmBankNumberError, setConfirmBankNumberError] = useState<string | null>(null);

  const [isShowOTP, setIsShowOTP] = useState<boolean>(false);
  const [isShowSuccessValidate, setIsShowSuccessValidate] = useState<boolean>(false);
  ...
  const filteredBanks = bankList?.bList?.filter((bank) =>
  ...
  const handleChangeBankAccount = async (): Promise<void> => {
    try {
      ...
      if (response?.data?.message === 'Terima kasih atas kesabarannya. Kami ingin memberitahukan bahwa masih terdapat permintaan perubahan data yang masih dalam proses. Mohon tunggu sampai proses ini selesai dan Anda dapat mengajukannya kembali.') {
        toast.warning(t(`${pathTranslation}.validation.text1`))
        setIsShowOTP(false)
      } else if (response?.data?.status === "Sukses" && response?.data?.statusCode === "200") {
        toast.success(t(`${pathTranslation}.validation.text2`))
        setIsAllowedOTP(true)
      } else if (response?.data?.message === 'No rekening salah atau tidak valid.') {
        setConfirmBankNumberError(t(`${pathTranslation}.validation.text5`))
        setIsShowOTP(false)
      } else if (response?.data?.message === 'Nama pemilik rekening tidak sesuai.') {
        setConfirmNewBankAccountNameError(t(`${pathTranslation}.validation.text6`))
        setIsShowOTP(false)
      } else if (response?.data?.message === 'Password salah, silakan coba kembali.') {
      ...
      if (error?.response?.data?.message === 'too many hits') {
        ...
        toast.error(t(`${pathTranslation}.validation.text8`))
      } else if (error?.response?.data?.messages?.message === 'Silahkan Pilih metode pengiriman Kode Verifikasi.') {
        setConfirmOtpTypeError(t(`${pathTranslation}.validation.text9`))
      } else if (error?.response?.data?.messages?.metodeVerifikasi === 'Silahkan Pilih Metode Verifikasi') {
        setConfirmOtpTypeError(t(`${pathTranslation}.validation.text9`))
      }
      setIsShowOTP(false)
      ...
      <div className="flex flex-col md:flex-row gap-2 md:gap-4">
        <div className="relative w-full" ref={dropdownRef}>
          ...
          >
            {(selectedBankCode?.length > 0)
              ? bankList?.bList?.find((bank) => bank?.code === selectedBankCode)?.name
          ...
          {isOpen && (
            <div className="absolute z-50 w-full bg-white border border-gray-300 rounded-md mt-1 max-h-60 overflow-auto shadow-lg">
              ...
              {filteredBanks?.map((bank) => (
                <div
                  ...
                  className={`px-4 py-2 hover:bg-blue-100 cursor-pointer ${
                    bank?.code === selectedBankCode ? "bg-blue-50 font-semibold" : ""
                  }`}
                  onClick={() => {
              ...
              {filteredBanks.length === 0 && (
                <div className="px-4 py-2 text-gray-500">{t(`${pathTranslation}.text7`)}</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="relative w-full">
          ...
          {(confirmBankNumberError !== null) && (
            <p className="text-sm text-red-500 mt-1 font-poppins">{confirmBankNumberError}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-2 md:gap-4">
        <div className="relative w-full">
          ...
          {(confirmNewBankAccountNameError !== null) && (
            <p className="text-sm text-red-500 mt-1 font-poppins">{confirmNewBankAccountNameError}</p>
          )}
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-2 md:gap-4">
        <div className="relative w-full md:w-1/2 md:pr-2">
          ...
          <select
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="" disabled>{t(`${pathTranslation}.text15`)}</option>
