...
const VirtualAccountStep: React.FC<VirtualAccountStepProps> = ({
  ...
  const [paymentInstruction, setPaymetnInstruction] =
    useState<PaymentInstruction>();
          ...
          <Card className="p-5 mt-4 bg-white font-poppins">
            ...
            >
              <h1 className="text-xl font-bold">
                {t('PlayPayment.VirtualAccountGuide.howToPay')}
              </h1>
              <button className="ml-2">{isOpen ? '▲' : '▼'}</button>
            </div>
            <div
            ...
            >
              <div className="mb-4">
                <label className="block text-gray-700 font-bold mb-2">
                  {t('PlayPayment.VirtualAccountGuide.choosePayment')}
                </label>
                <select
                  className="w-full p-2 border rounded-md cursor-pointer"
                  value={selectedChannelIndex}
                  onChange={e => {
                    setSelectedChannelIndex(Number(e.target.value));
                  }}
                >
