...
import { createPostPlay } from '@/repository/play.repository';
...
interface props {
  ...
  setIsLoadingPost?: (isLoadingPost: boolean) => void;
  dataPost?: PostData;
  setDataPost?: any;
  setFilter?: any;
  setData?: any;
...
const ModalMention: React.FC<props> = ({
  ...
  const [dollarLists, setDollarLists] = useState<DollarList[]>([]);
  ...
  useEffect(() => {
    if (dataPost !== undefined) {
      setForm(prevState => ({
        ...
        //   canAddNewOption: dataPost.pooling_new_option,
        //   endDate:
  ...
  const handlePostCircle = async (event: any): Promise<void> => {
    ...
    try {
      ...
      if (form.pie_title !== '') {
        ...
        payload.pie_amount = form.pie_amount;
      }

      if (form.privacy === 'premium') {
        payload.premium_fee = form.premium_fee;
      }
      ...
      toast.error(`Error posting or editing: ${error as string}`);
    } finally {
      setIsLoading(false);
    }
  };
