...
import { Server, Keypair, TransactionBuilder, Operation, BASE_FEE, Networks, Asset } from 'diamnet-sdk';
...
export const createSellOffer = async (account, sellingAsset, buyingAsset, amount, price, offerId = '0') => {
  try {
    const transaction = new TransactionBuilder(account, {
      fee: BASE_FEE,
      networkPassphrase: networkPassphrase,
...
export const createTrustline = async (account, assetCode, assetIssuer, limit = undefined) => {
  try {
...
const handleConnectWallet = async () => { // This feature just for debug only, pls delete when launch on product
  try {
...
export const connectWallet = async () => {
    try {
        if (!window.diam) {
            throw new Error('DIAM Wallet extension is not installed. Please install it to proceed.');
        }
