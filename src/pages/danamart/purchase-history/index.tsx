...
const PurchaseHistory = (): React.ReactElement => {
  ...
  const [isShowModalPurchaseInformation, setIsShowModalPurchaseInformation] = useState<boolean>(false);
                ...
                <table className="min-w-full border border-gray-300 rounded-md">
                  <thead>
                    <tr className="text-[#27A590] font-poppins bg-[#DCFCE4]">
                      <th className="text-center p-2 border-b border-gray-300 cursor-pointer" onClick={() => { handleSort('kodeEfek'); }}>{t(`${pathTranslation}.table.text6`)}</th>
                      <th className="text-center p-2 border border-gray-300 cursor-pointer" onClick={() => { handleSort('penawaranId'); }}>{t(`${pathTranslation}.table.text7`)}</th>
                      <th className="text-center p-2 border border-gray-300 cursor-pointer" onClick={() => { handleSort('jmlPembelian'); }}>{t(`${pathTranslation}.table.text8`)}</th>
                      <th className="text-center p-2 border border-gray-300 cursor-pointer" onClick={() => { handleSort('ModalPembelian'); }}>{t(`${pathTranslation}.table.text9`)}</th>
                  ...
                  <tbody>
                    {paginatedData?.length > 0 ? (
                      paginatedData?.map((purchases, index) => (
                        ...
                        >
                          <td className="p-2 border border-gray-300 text-center">{purchases?.kodeEfek}</td>
                          <td className="p-2 border border-gray-300 text-center">{purchases?.modalPembiayaan.idPenawaran}</td>
                          <td className="p-2 border border-gray-300 text-right">{purchases?.ModalPembelian?.jmlPembelian !== '-' ? `IDR ${standartCurrency(convertCurrencyToNumber(purchases?.ModalPembelian?.jmlPembelian ?? '') ?? 0)}` : purchases?.ModalPembelian?.jmlPembelian}</td>
                          <td className="p-2 border border-gray-300">{purchases?.ModalPembelian?.tglPembelian}</td>
                                      ...
                                      <div
                                        ...
                                        onClick={async() => {
                                          ...
                                          if (option?.action === 'redirect') {
                                            await router.push(`/danamart/offer/prospectus/${purchases?.penawaranId}`)
                                          }
                                        }}
                                      >
                                        {option?.title}
