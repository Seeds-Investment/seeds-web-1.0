  ...
  const [thumbnailList, setThumbnailList] = useState<any>([]);
                ...
                .map((part, partIndex) => {
                  if (
                    ...
                    const extractedValue =
                      matchResult !== null ? matchResult[1] : null;
                    return (
                      <button
                        key={index}
                        onClick={() => {
                    ...
                    part.match(/\b(?:https?|ftp):\/\/\S+|\b(?:www\.\S+)\b/) !==
                    null
                  ) {
                    linkUrl = part;
                    const link = part.startsWith('www.')
                      ? `http://${part}`
                      : part;
                    return (
                      <button
            ...
            if (checkPostOrderPlay(dataPost?.content_text)) {
              ...
              const promises = res?.map(async (el: any) => {
                if (el?.includes('-circle') === true) {
                  ...
                  await getPlayById(el?.replace('-play', '')).then(
                    (res: any) => {
                  ...
                  await getAssetById(el?.replace('-asset', '')).then(
                    (res: any) => {
                      tempThumbnailList.push({
                        thumbnailType: 'asset',
                        ...res.marketAsset
              ...
              Promise.all(promises)
                .then(() => {
                  const filteredThumbnailList =
                    removeDuplicateIds(tempThumbnailList);
          ...
          const onPressTag = (content: any): void => {
            if (content?.includes('-people') === true) {
              ...
              router
                .push(
                  `/homepage/assets/${content?.replace('-asset', '') as string}`
                )
                .catch(err => {
                  console.error(err);
                });
              ...
              router
                .push(`/social/search?hashtags=${content as string}`)
                .catch(err => {
                  console.error(err);
                });
            }
          ...
          const toDetailTag = useCallback((item: any) => {
            if (item?.thumbnailType === 'circle') {
              ...
              router
                .push(`/play/tournament/${item?.id as string}`)
                .catch(err => {
                  console.error(err);
                });
              ...
              router
                .push(`/homepage/assets/${item.id as string}`)
                .catch(err => {
                  console.error(err);
                });
            }
                ...
                setData((prevDataPost: any | null) => {
                  if (prevDataPost !== null) {
                    if (Array.isArray(prevDataPost)) {
                      ...
                      const filteredTrue = newData.filter(
                        el => el.is_pinned === true
                      );
                      const filterPin = filteredTrue.filter(
                        el => el.id !== dataPost.id
                      );
                      const findPin = filteredTrue.find(
                        el => el.id === dataPost.id
                      );
          ...
          const handleSetChartData = (): void => {
            ...
            dataPost.pie.forEach((item: any) => {
              ...
              convertedData.datasets[0].backgroundColor.push(
                generateRandomColor()
              );
            });
          ...
          const redirectToPaymentPostPremium = (): void => {
            router
              .push(
                isGuest() ? '/auth' : `/social/payment/${dataPost.id as number}`
              )
              .catch(error => {
          ...
          return (
            <>
              <div
                ...
                key={`${dataPost.id as string}${Math.floor(
