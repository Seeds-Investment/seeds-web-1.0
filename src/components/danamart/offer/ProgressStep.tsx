import {
  ProgressBusinessGreen,
  ProgressBusinessGrey,
  ProgressDistributionGreen,
  ProgressDistributionGrey,
  ProgressHandingCashGreen,
  ProgressHandingCashGrey,
  ProgressHandingSecurityGreen,
  ProgressHandingSecurityGrey,
  ProgressListingGreen,
  ProgressListingGrey,
  ProgressPrelistingGreen,
  ProgressPrelistingGrey,
  ProgressSigningGreen,
  ProgressSigningGrey
  ...
  useEffect(() => {
    if (timelineOffer?.Status === '7') {
      if (timelineOffer?.status_sla === 'Pre-Listing') {
        ...
        timelineOffer?.status_sla === 'Listing' ||
        timelineOffer?.status_sla === 'Pendanaan Selesai'
      ) {
        setCurrentStep(2);
            ...
            {[
              {
                ...
                color: currectStep > 0 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 1 ? ProgressListingGreen : ProgressListingGrey,
                label:
                  timelineOffer?.status_sla !== 'Pendanaan Selesai'
                    ? t(`${pathTranslation}.text2`)
                    : t(`${pathTranslation}.text3`),
                color: currectStep > 1 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 2
                ...
                color: currectStep > 2 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 3 ? ProgressSigningGreen : ProgressSigningGrey,
                label: t(`${pathTranslation}.text5`),
                color: currectStep > 3 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 4
                ...
                color: currectStep > 4 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 5
                ...
                color: currectStep > 5 ? '#4FE6AF' : '#BDBDBD'
              },
              {
                src:
                  currectStep > 6
                    ? ProgressBusinessGreen
                    : ProgressBusinessGrey,
                label: getLastStepName(),
                color: currectStep > 6 ? '#4FE6AF' : '#BDBDBD'
              }
            ].map((item, index) => (
              <div
                key={index}
              ...
              >
                {hoveredIndex === index &&
                  timelineOffer?.beforeTimeline[index] !== '-' && (
                ...
                <div
                  className={`flex items-center justify-center w-[50px] h-[50px] bg-white border-2 rounded-full p-2 cursor-pointer`}
