import { BankBCA, BankBNI, BankBRI, BankMandiri, BankPermata, SeedyPayment } from '@/assets/danamart';
...
import { cancelPayment, getDetailPayment } from '@/repository/danamart/offers.repository';
...
const PaymentVA = (): React.ReactElement => {
  ...
  const [isShowCancelPayment, setIsShowCancelPayment] = useState<boolean>(false);
  ...
  const selectedBank = paymentDetail?.Data.kode_pembayaran as keyof typeof bankInstructions;
  ...
  useEffect(() => {
    if (pinjamanId !== undefined) {
      void Promise.all([
        getDataDetailPayment(pinjamanId)
      ])
    }
    ...
    <PageGradient defaultGradient className="w-full">
      <div className="w-full flex flex-col justify-center items-center rounded-xl font-poppins p-5 bg-white">
          ...
          <div className='w-full flex flex-col gap-4'>
            ...
            <div>
              ...
              </Typography>
              {
                paymentDetail !== undefined ?
                  <Typography className='font-poppins text-[#262626] font-medium'>
                    IDR {standartCurrency(paymentDetail?.Data?.jml_pendanaan ?? 0)}
                  </Typography>
                  :
                  <div className="w-[80%] md:w-[60%] h-[18px] flex flex-col justify-start items-start bg-gray-300 animate-pulse rounded-full">
                    <div className="text-sm text-center items-center bg-gray-400 flex-grow w-full rounded-full" />
                ...
                paymentDetail !== undefined ?
                  <div className='flex justify-center items-center w-[200px] h-auto mt-2'>
                    <Image
                      ...
                      width={100}
                      height={100}
                      src={handleBankLogo(paymentDetail?.Data?.kode_pembayaran ?? '')}
                      className="h-full w-auto"
                    />
                  </div>
                  :
            ...
            <div>
              ...
              </Typography>
              {
                paymentDetail !== undefined ?
                  <Typography
                    onClick={async() => { await handleCopyVANumber(paymentDetail?.Data?.fva ?? '')}}
        ...
        <div className='w-full flex flex-col md:flex-row gap-2 justify-between items-center mt-4'>
          <Button
            onClick={() => { setIsRefetch(!isRefetch) }}
            disabled={isLoading}
          ...
          <Button
            onClick={() => { setIsShowCancelPayment(true) }}
            disabled={isLoading}
        ...
        <div className='mt-6'>
          <Typography className="text-md font-semibold font-poppins text-[#7C7C7C] mb-2">
            {t(`${pathTranslation}.howToPayVA1`)} {paymentDetail?.Data?.kode_pembayaran} {t(`${pathTranslation}.howToPayVA2`)}
          </Typography>
          <div className='mt-2'>
