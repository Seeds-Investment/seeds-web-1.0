...
const QuizWinnerSection: React.FC<QuizWinnerProps> = ({ detailQuiz, preferredCurrency }) => {
        ...
        (detailQuiz?.prize_type === 'LINK') &&
          <div className="mt-4">
            <div className="text-lg font-semibold">{t('quiz.quizPrize')}</div>
            {
              ((detailQuiz?.winner_link_url !== null) && (detailQuiz?.winner_link_url[0] === '')) &&
                <div className="rounded-lg overflow-hidden relative flex justify-center items-center bg-seeds-button-green">
                  ...
                  <div
                    className='absolute top-1 flex justify-center items-center gap-2 text-md w-[100px] h-[30px] text-center rounded-full font-semibold text-white bg-seeds-button-green border-2 border-seeds-green'
                  >
            ...
            <div className="flex flex-col gap-2 justify-start items-center mt-4 pb-8 md:pb-0 md:grid md:grid-cols-2 w-full">
              {
                detailQuiz?.winner_image_url?.slice(1).map((item: string | number, i: number) => {
                  ...
                  const linkUrl =
                    Array.isArray(detailQuiz?.winner_link_url) && adjustedIndex >= 0 && adjustedIndex < detailQuiz.winner_link_url.length
                  ...
                  const titleSuffix = i >= 0 && i < suffixArray.length ? suffixArray[i] : 'th';
                  const containerClasses = 'w-full h-[150px] flex justify-center items-center border-2 rounded-lg overflow-hidden';
                  const emptyClasses = 'w-full h-[150px] rounded-lg border-2 flex justify-center items-center overflow-hidden border-2 border-seeds-button-green';

                  return (
                    <div
                      key={adjustedIndex}
                      className={`${isPrimary ? 'text-md' : 'text-sm'} flex flex-col justify-center items-center w-full relative text-white`}
                    >
                      <div
                        className="h-[150px] w-full flex justify-center items-center rounded-lg bg-seeds-button-green"
                      >
                        {(imageUrl.length > 0) ? (
                          ...
                          ((linkUrl !== null) && (linkUrl !== "")) ? (
                            <a href={linkUrl} target="_blank" className={emptyClasses} />
                          ) : (
                      ...
                      <div
                        className='absolute top-1 flex justify-center items-center gap-2 text-md w-[100px] h-[30px] text-center rounded-full font-semibold border-2 border-seeds-green bg-seeds-button-green'
                      >
                        {
