            ...
            !isLoadingHistory ? (
              earningHistory !== null ? (
                      ...
                      >
                        <div className='flex justify-start items-center gap-2 md:gap-4'>
                          <div className={`${(item?.direction ?? 'IN') === 'IN' ? 'bg-[#DCFCE4]' : 'bg-[#FFF7D2]'} w-[40px] h-[40px] md:w-[60px] md:h-[60px] flex justify-center items-center rounded-full`}>
                            <Image
                              alt=""
                              src={(item?.direction ?? 'IN') === 'IN' ? EarningCoin : WithdrawCoin}
                              width={100}
                              height={100}
                          ...
                          <div className=''>
                            <Typography className='text-xs md:text-base font-semibold font-poppins'>
                              {(item?.direction ?? 'IN') === 'IN' ? t('earning.earnings') : t('earning.withdrawal')}
                            </Typography>
                            <Typography className='md:hidden text-xs md:text-base font-poppins text-[#BDBDBD]'>
                              {(item?.source_name ?? 'Winner Rewards')?.length < 20 ? (item?.source_name ?? 'Winner Rewards') : `${(item?.source_name ?? 'Winner Rewards')?.slice(0, 19)}...`}
                            </Typography>
                ...
                <div className="bg-white flex flex-col justify-center items-center text-center lg:px-0">
                  ...
                  <p className="text-[#7C7C7C]">{t('earning.noDataDescription')}</p>
                </div>
              )
            ) : (
