...
import useRequestWithdrawal, { type RequestWithdrawI } from '@/hooks/danamart/useRequestWithdrawal';
...
const ModalWithdraw: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (withdrawValue >= 5000) {
      setValue('dana_diterima', `Rp. ${(withdrawValue - adminFee)?.toLocaleString('id-ID')}`);
    } else {
    ...
    try {
      ...
      handleSubmit(async (data: RequestWithdrawI) => {
        try {
          ...
          if (response?.message === 'Selamat ya penarikan dana Anda diterima dan akan segera diproses!') {
          ...
          setIsLoading(false);
          if (error?.response?.data?.messages?.message === 'Masih ada proses tarik dana yang belum diproses di akun Kamu. Silakan tunggu proses tarik dana hingga selesai, lalu coba kembali ya.') {
  ...
  return (
    ...
    >
        ...
        requestStep === 'form' ?
          <>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: t(`${pathTranslation}.steps`) ?? '' }}
            />
          ...
          <div className='md:px-4 pb-4'>
            ...
            <div className="my-4">
              <Typography className="mb-2">{t(`${pathTranslation}.text6`)}</Typography>
              <Select value={otpType} onChange={(value) => { setOtpType(value as 'wa' | 'sms'); }}>
      ...
      <div className="w-full mt-2 flex flex-col md:flex-row justify-center md:justify-end gap-4 md:gap-2">
        <Button
          onClick={() => {
            requestStep === 'form' ? setIsShowModalWithdraw(!isShowModalWithdraw) : setRequestStep('form')
          }}
        ...
        >
          {requestStep === 'form' ? t(`${pathTranslation}.text9`) : t(`${pathTranslation}.text10`)}
        </Button>
        <Button
          disabled={
