...
const useDownloadReport = (): any => {
  ...
  const onSubmit = async (formData: { daritgl: string; sampaitgl: string; file: string }): Promise<void> => {
    try {
      const payload = { ...formData };
      const jsonString = JSON.stringify(payload);
      const encryptedData = encryptRequest(jsonString) ?? "";

      const reportUrl = `https://dev.danamart.id/development/dm-scf-api/public/pemodal/DanaMasuk/laporanDanaMasuk?data=${encodeURIComponent(encryptedData)}`;
