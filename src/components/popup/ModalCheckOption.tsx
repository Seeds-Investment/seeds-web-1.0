...
const ModalCheckOption: React.FC<Props> = ({ onClose, ticketId, ticketStatus }) => {
  ...
  const handleCheckInOut = async (ticketId: string): Promise<void> => {
    try {
      ...
      if ((ticketStatus === 'CHECKED_IN') || (ticketStatus === 'ISSUED')) {
        ...
        void router.replace(`/homepage/event/${id as string}/check-in-out`)
          .then(() => {
            router.reload();
          }
        );
  ...
  return (
    ...
    >
      ...
      <div className='w-full flex gap-2 justify-center items-center mt-4'>
        <button onClick={() => { onClose()}} className='w-full bg-[#DD2525] py-2 rounded-full text-white font-poppins font-semibold cursor-pointer'>
          {t('seedsEvent.ticket.no')}
        </button>
        <button
          onClick={() => { void handleCheckInOut(ticketId) }}
