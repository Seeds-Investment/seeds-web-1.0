...
const Promotion = (): React.ReactElement => {
  ...
  const handleValidatePromoCode = async (): Promise<void> => {
    try {
      ...
      if (response?.message === 'Selamat! Kode-mu valid dan berhasil diklaim. Saat ini sudah bisa menikmati benefitnya.') {
      ...
      if (error?.response?.data?.message === 'Maaf! Kode-mu tidak valid, kuota sudah habis , masa berlakunya telah berakhir atau tidak memenuhi syarat dan ketentuan. Silakan coba lagi dengan kode lainnya ya.') {
  ...
  return (
    <PageGradient defaultGradient className="w-full">
      <div className="w-full bg-white flex flex-col px-5 py-5 rounded-lg">
        ...
        <div
          onClick={() => { setIsShowPromoGuide(true) }}
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
          <label className="w-full flex flex-col">
            ...
            <select
              value={jnsPromo}
              onChange={(e) => { setJnsPromo(e.target.value); }}
              className="py-2 px-4 border rounded-full"
            >
              <option value="" disabled>{t(`${pathTranslation}.text3`)}</option>
