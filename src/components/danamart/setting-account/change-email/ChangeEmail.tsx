    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        if (decryptedData !== null) {
          ...
          if (decryptedDataObject?.message === 'Silakan cek email Kamu untuk verifikasi perubahan email.') {
      ...
      if (error?.response?.data?.messages?.error === 'Password salah, silakan coba kembali.') {
        setConfirmPasswordError(t(`${pathTranslation}.text6`))
      } else if (error?.response?.data?.messages?.email === 'Alamat Email sudah digunakan!, Silahkan Ganti Email yang lain') {
  ...
  return (
    <div className="flex flex-col gap-4 mt-6 mb:mt-8 mb-2">
      ...
      <Typography className="font-poppins text-md text-[#262626]">
        ...
        <span className="font-semibold">{userProfileData?.User?.email ?? ''}</span>
      </Typography>
