...
import { getEarningBalance, requestWithdraw } from '@/repository/earning.repository';
...
const WithdrawValue = (): React.ReactElement => {
  ...
  const [form, setForm] = useState<IRequestWithdraw>({
    ...
    nett_amount: 0,
    fee: bankAccount?.admin_fee + bankAccount?.service_fee - (bankAccount?.is_promo_available ? bankAccount?.promo_price : 0)
  });
  ...
  const handleInputWithdrawValue = (event: React.ChangeEvent<HTMLInputElement>): void => {
          ...
          <div className='bg-[#E9E9E9] p-4 rounded-lg'>
            <div className='flex gap-2'>
              ...
              <input
                ...
                name="search"
                value={withdrawValue}
                placeholder={`${userInfo?.preferredCurrency ?? 'IDR'} - ${t('earning.nominalTransfer')}`}
                onChange={e => {
            ...
            <div className='w-full text-[#3C49D6] text-sm'>
              {t('earning.minimumWithdrawal')} {userInfo?.preferredCurrency ?? 'IDR'}{standartCurrency(10000).replace('Rp', '')}
            </div>
          </div>
