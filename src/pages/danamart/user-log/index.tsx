          ...
          !isLoading ?
            <div className='relative'>
              <div className="w-full flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                ...
                <Button
                  onClick={() => { void getUserLogData(params) }}
                ...
                <table className="min-w-full border border-gray-300 rounded-md">
                  <thead>
                    <tr className="text-[#27A590] font-poppins bg-[#DCFCE4]">
                      <th className="text-center p-2 border-b border-gray-300 cursor-pointer" onClick={() => { handleSort('index'); }}>#</th>
                      <th className="text-center p-2 border-b border-gray-300 cursor-pointer" onClick={() => { handleSort('datetime'); }}>Tanggal</th>
                      <th className="text-center p-2 border border-gray-300 cursor-pointer" onClick={() => { handleSort('jenis_aksi'); }}>Aktivitas</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedData?.length > 0 ? (
                      paginatedData?.map((userLog, i) => {
                        const displayedIndex = sortColumn === 'index' && sortOrder === 'desc'
                          ? paginatedData?.length - i
                          : i + 1;

                        return (
                          <tr key={i} className="hover:bg-gray-50">
                            <td className="p-2 border border-gray-300 text-center">{displayedIndex}</td>
                            <td className="p-2 border border-gray-300">{userLog?.datetime}</td>
                            <td className="p-2 border border-gray-300">{userLog?.jenis_aksi}</td>
                          </tr>
                        );
                      })
                    ) : (
