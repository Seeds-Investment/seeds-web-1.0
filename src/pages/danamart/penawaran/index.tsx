...
const NextArrow = (props: ArrowProps): React.ReactElement => {
	...
	return (
		<div
      className={`${className ?? ''} rounded-full absolute right-[-50px] top-1/2 transform -translate-y-1/2 cursor-pointer shadow-lg`}
...
const PrevArrow = (props: ArrowProps): React.ReactElement => {
	...
	return (
		<div
      className={`${className ?? ''} rounded-full absolute left-[-50px] top-1/2 transform -translate-y-1/2 cursor-pointer shadow-lg`}
			style={{
				left: 10, background: '#4FE6AF'
			}}
			onClick={onClick}
		/>
	);
          ...
          <Typography className="font-poppins text-md">
            {t(`${pathTranslation}.text2`)}
				...
				<div className='w-full flex flex-col md:flex-row gap-4'>
					<div className="w-full flex flex-col gap-2">
						<Typography className="font-poppins font-medium">{t(`${pathTranslation}.filter.text1`)}</Typography>
						<select
						...
						>
							...
							<option value="Saham">{t(`${pathTranslation}.filter.text3`)}</option>
							<option value="Obligasi">{t(`${pathTranslation}.filter.text4`)}</option>
						</select>
					</div>
					<div className="w-full flex flex-col gap-2">
						<Typography className="font-poppins font-medium">{t(`${pathTranslation}.filter.text5`)}</Typography>
						<select
						...
						>
							...
							<option value="Listing">{t(`${pathTranslation}.filter.text7`)}</option>
							<option value="Prelisting">{t(`${pathTranslation}.filter.text8`)}</option>
						</select>
					</div>
					<div className="w-full flex flex-col gap-2">
						<Typography className="font-poppins font-medium">{t(`${pathTranslation}.filter.text9`)}</Typography>
						<select
						...
						>
							...
							<option value="Jumlah_Des">{t(`${pathTranslation}.filter.text11`)}</option>
							<option value="Jumlah_Asc">{t(`${pathTranslation}.filter.text12`)}</option>
							<option value="Jangka_Desc">{t(`${pathTranslation}.filter.text13`)}</option>
							<option value="Jangka_Asc">{t(`${pathTranslation}.filter.text14`)}</option>
							<option value="Bagi_Desc">{t(`${pathTranslation}.filter.text15`)}</option>
							<option value="Bagi_Asc">{t(`${pathTranslation}.filter.text16`)}</option>
						</select>
					</div>
				</div>
				<div className="w-full my-5 h-fit cursor-pointer gap-2">
					{
						loading ?
							...
							:
							<Slider {...sliderSettings} className='custom-danamart-slider w-full h-fit gap-2 pt-2 pb-8 md:pb-4'>
				...
				<div className='w-full flex flex-col md:flex-row gap-4 items-stretch'>
					{["Prelisting", "Listing"].map((title, index) => (
						<div key={index} className="relative p-[2px] rounded-lg flex-1 flex">
							...
							<div className="relative bg-white rounded-lg p-5 flex flex-col h-full">
								<div className={`absolute top-0 left-0 bg-gradient-to-r ${
									title === "Prelisting" ? "from-[#48C0AB] to-[#B798FF]" : "from-[#66E3B6] to-[#5987CB]"
								...
								<div className="mt-12 flex-1">
									{title === "Prelisting" ? (
										<div 
											...
											dangerouslySetInnerHTML={{ __html: t(`${pathTranslation}.prelisting`) ?? '' }} 
										/>
									) : (
										<div 
											className="font-poppins" 
											dangerouslySetInnerHTML={{ __html: t(`${pathTranslation}.listing`) ?? '' }} 
										/>
									)}
								</div>
							</div>
