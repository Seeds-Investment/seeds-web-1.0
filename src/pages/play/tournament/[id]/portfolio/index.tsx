...
const Portfolio = (): React.ReactElement => {
  ...
  const [portfolioActiveTab, setPortfolioActiveTab] = useState(PortfolioFilter.OVERVIEW);
  ...
  const COLORS = ['#F44336', '#E81E63', '#9C27B0', '#673AB7', '#3F51B5', '#2196F3', '#03A9F4', '#00BCD4', '#009688', '#4CAF50', '#8BC34A', '#CDDC39', '#FFEB3B', '#FFC107', '#FF9800', '#FF5722', '#795548', '#9E9E9E', '#607D8B', '#000000',];
    ...
    <>
      <div className='w-full flex flex-col justify-center items-center rounded-xl font-poppins p-5 bg-white'>
        ...
        <div className='w-full p-5 bg-gradient-to-br from-[#50D4B2] from-50% to-[#E2E2E2] rounded-xl h-[150px] relative overflow-hidden mt-4'>
          ...
          <Image alt="" src={BannerCircle} className='absolute top-0 right-0 z-10'/>
        </div>
        ...
        <div className='w-full xl:w-[80%] md:mt-4 mb-4 flex flex-wrap gap-4 justify-center items-center'>
          {data.map((datas, index) => (
            <div key={index} className='flex gap-2'>
              <div style={{ backgroundColor: COLORS[index % COLORS.length] }} className='w-[20px] h-[20px] rounded-md'/>
              <div className='font-semibold'>
                {datas.name} /
              </div>
              <div>
              ...
              <div className='text-seeds-button-green'>
                +25%
              </div>
            </div>
          ))}
        ...
        <div className='w-full mt-4'>
          ...
          <div className='w-full flex items-start justify-start mt-4'>
            <div className="flex flex-row items-center gap-3 max-w-full overflow-x-auto no-scroll">
              {statusPortfolio.map(item => (
                ...
                >
                  <Image alt="" src={item.status === portfolioActiveTab ? item.iconActive : item.iconInactive} className='w-[20px]'/>
                  <div className='mr-4'>
                    {item.title}
                  </div>
                </button>
          ...
          <div className='flex flex-col'>
            <div onClick={async() => await router.push('play/tournament/1/portfolio/detail-portfolio')} className='flex justify-between items-center p-4 mt-4 bg-[#F9F9F9] md:bg-white border border-[#E9E9E9] md:border-none rounded-lg'>
