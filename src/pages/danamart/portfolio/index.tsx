                    ...
                    {paginatedData?.length > 0 ? (
                      paginatedData?.map((portfolio, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="p-2 border border-gray-300 text-right">{portfolio?.hargaPerlembarSaham !== '-' ? `${userData?.preferredCurrency ?? 'IDR'} ${standartCurrency(convertCurrencyToNumber(portfolio?.hargaPerlembarSaham ?? '') ?? 0)}` : portfolio?.hargalembarSaham}</td>
                          ...
                          <td className="p-2 border border-gray-300 text-right">{`${userData?.preferredCurrency ?? 'IDR'} ${standartCurrency(convertCurrencyToNumber(portfolio?.modalInfoPendanaan?.jumlahPembelian ?? '') ?? 0)}`}</td>
