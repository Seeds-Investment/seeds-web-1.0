...
interface Props {
  setIsShowMonthlyIncomeInfo: React.Dispatch<React.SetStateAction<boolean>>;
}

const ModalMonthlyIncomeInformation: React.FC<Props> = ({
  setIsShowMonthlyIncomeInfo
}) => {
  ...
  return (
    ...
    >
      <button
        onClick={() => {
          setIsShowMonthlyIncomeInfo(false);
        }}
      ...
      <Typography className="font-bold text-lg font-poppins md:w-[90%] mb-4">
        {t(`${pathTranslation}.popUpLabel.monthlyIncomeTitle`)}
      </Typography>
      ...
      <div
        ...
        dangerouslySetInnerHTML={{
          __html:
            t(
              `${pathTranslation}.popUpLabel.monthlyInformation`
            ) ?? ''
        }}
      />
...
export default ModalMonthlyIncomeInformation;
